...
fetch_from_array:
    ...
    if (EXPECTED(Z_TYPE_P(container) == IS_STRING)) {
        ...
        if (UNEXPECTED(ZMARK_ISERR(container))) {
#if PHP_VERSION_ID < 70100
            ZVAL_INDIRECT(result, &EG(error_zval));
#else
            ZVAL_ERROR(result);
...
static int php_zmark_binary_assign_op_helper(binary_op_type binary_op, zend_execute_data *execute_data) /* {{{ */ {
    ...
    if (opline->op1_type == IS_VAR) {
        if (var_ptr == NULL || ZMARK_ISERR(var_ptr)) {
    ...
    if (ZMARK_RET_USED(opline)) {
        ZVAL_COPY(EX_VAR(opline->result.var), var_ptr);
    }

    if ((ZMARK_OP1_TYPE(opline) & (IS_VAR|IS_TMP_VAR)) && free_op1) {
        zval_ptr_dtor_nogc(free_op1);
    }

    if ((ZMARK_OP2_TYPE(opline) & (IS_VAR|IS_TMP_VAR)) && free_op2) {
...
static void php_zmark_register_opcode_handlers()
{
...
PHP_FUNCTION(zclear)
{
    ...
    ZVAL_DEREF(z_str);
    if (IS_STRING != Z_TYPE_P(z_str) || Z_STRLEN_P(z_str) == 0 || !ZCHECK_FLAG(Z_STR_P(z_str))) {
        RETURN_FALSE;
    }

    ZCLEAR_FLAG(Z_STR_P(z_str));
    RETURN_TRUE;
}
...
PHP_MINIT_FUNCTION(zmark)
{
    ...
    /* 兼容php7.4 */
    REGISTER_LONG_CONSTANT("ZMARK_ASSIGN_CONCAT", ZMARK_ASSIGN_CONCAT, CONST_CS|CONST_PERSISTENT);
...
ZEND_BEGIN_ARG_INFO_EX(zid_arginfo, 0, 0, 1)
    ZEND_ARG_INFO(1, z)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_INFO_EX(zmark_arginfo, 0, 0, 1)
    ZEND_ARG_INFO(1, string)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_INFO_EX(zclear_arginfo, 0, 0, 1)
    ZEND_ARG_INFO(1, string)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_INFO_EX(zcheck_arginfo, 0, 0, 1)
    ZEND_ARG_INFO(0, string)
ZEND_END_ARG_INFO()

/* {{{ zmark_functions[]
 *
 * Every user visible function must have an entry in zmark_functions[].
 */
const zend_function_entry zmark_functions[] = {
    PHP_FE(zid, zid_arginfo)
    PHP_FE(zmark, zmark_arginfo)
    PHP_FE(zclear, zclear_arginfo)
    PHP_FE(zcheck, zcheck_arginfo)
    PHP_FE(zrename_function, NULL)
    PHP_FE(zrename_class, NULL)
    PHP_FE(zregister_opcode_callback, NULL)
    PHP_FE_END    /* Must be the last line in zmark_functions[] */
};
/* }}} */
...
zend_module_entry zmark_module_entry = {
    ...
    PHP_MINIT(zmark),
    PHP_MSHUTDOWN(zmark),
    PHP_RINIT(zmark),        /* Replace with NULL if there's nothing to do at request start */
    PHP_RSHUTDOWN(zmark),    /* Replace with NULL if there's nothing to do at request end */
    PHP_MINFO(zmark),
    PHP_ZMARK_VERSION,
